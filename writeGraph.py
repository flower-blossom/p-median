...
def draw(x, y, solution, depot):
    X = []
    Y = []
    solutionList = solution.solutionList
    coorDepot = x[depot - 1], y[depot - 1]
    ...
    kwargs = dict(label="Depot", zorder=3, marker="*", s=100)
    ax.scatter(*coorDepot ,c="tab:red", **kwargs) 
    ax.set_title(f" Total distance: {round(solution.costFuction)}")
...
def drawInteractive(x, y, solutionList, distanceMatrix):
    ...
    for idxCluster in range(len(solutionList)):
        ...
        PairOfPoints = findRadiusPoint(
            solutionList[idxCluster], distanceMatrix)
        print(PairOfPoints)
        if len(PairOfPoints) == 2:
            firstPoint = PairOfPoints[0]
            secondPoint = PairOfPoints[1]
            name = "Radius cluster: " + str(idxCluster)
            fig.add_scatter(x=[x[firstPoint], x[secondPoint]], y=[
                            y[firstPoint], y[secondPoint]], name=name)

    fig.update_traces(marker=dict(size=20))
    plotly.offline.plot(
        fig, filename='D:\Hoc\\thuc tap\cluster\image\\test.html')
    # fig.show()
